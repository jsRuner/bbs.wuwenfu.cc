	...
	if(menu) {
		...
		switch(tag) {
			...
			case 'aud':
				...
				str = '<div id="music_search_bar"><input class="inp_txt" name="" title="输入想要添加的歌曲名、专辑名、艺人名" value="" id="search_input"/><a class="btn_sm_m" href="javascript:void(0)" id="search_music_bt" title="插入" lowsrc="source/plugin/xhuaian_xiami/images/f02332.gif" onFocus="this.blur()" unselectable="on"><span>插入</span></a><p class="xg2 pbn">您可以去网易云音乐找到你的喜欢的歌曲，复制歌曲链接例如 <br> http://music.163.com/#/song?id=394718 粘贴到这里。<br> 点击“插入”即可与广大网友分享该歌曲</p><ul class="music_list" id="music_list"></ul><div id="self_set_pager_ui"></div></div>';
				break;
			case 'vid':
...
if($('search_input')) {	
	_attachEvent($('search_input'),'focus', function(e){
		e = e ? e : event;
		obj = BROWSER.ie ? event.srcElement : e.target;
		if(obj.value == obj.title) obj.value = '';
	});
	_attachEvent($('search_input'),'blur', function(e){
		e = e ? e : event;
		obj = BROWSER.ie ? event.srcElement : e.target;
		if ( !obj.value ) obj.value = obj.title;
	});
	_attachEvent($('search_input'),'keypress', function(e){
		e = e ? e : event;
		obj = BROWSER.ie ? event.srcElement : e.target;
		// 按Enter搜索
		if ( e.which == 13 ) {
			$('search_input').value ? searchMusicList(1) : $('search_input').value=$('search_input').title;
		}
	});
	_attachEvent($('search_music_bt'),'click', function(e){
		e = e ? e : event;
		obj = BROWSER.ie ? event.srcElement : e.target;
		// 按Enter搜索
		if ($('search_input').value != $('search_input').title) {
