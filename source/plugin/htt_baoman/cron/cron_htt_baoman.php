<?php
/**
 *    [htt_baoman(.cron_htt_baoman)] (C)2016-2099 Powered by 吴文付.
 *    Version: 1.0
 *    Date: 2016-4-2 16:24
 *    Warning: Don't delete this comment
 *
 *
 *    cronname: cron_htt_baoman
 *    week: -1
 *    day:-1
 *    hour:-1
...
$threads = $var['htt_baoman']['threads']; //采集数量。
$times = $var['htt_baoman']['times']; //采集时间间隔。

// $times = 5; //修改为5分钟。方便测试。

$charset_num = $var['htt_baoman']['charset'];  // 1utf-8 2gbk
$caiji_model = $var['htt_baoman']['caiji_model']; //1纯文 2表示纯图 3图文
$check = $var['htt_baoman']['check'];  //1不审核 2审核。
$title_default = $var['htt_baoman']['title_default']; //默认标题
$post_model = $var['htt_baoman']['post_model']; //发帖模式
...
if (!empty($log_last) && $log_last['num'] == 0 ) {
    ...
    //删除日志记录。
    C::t('#htt_baoman#htt_baoman_log')->delete($log_last['id']);
}
...
//数据源。
$urls = array(
    'baoman'=>'http://baozoumanhua.com/baoman',
    'duanpian'=>'http://baozoumanhua.com/catalogs/duanpian',
    'lianzai'=>'http://baozoumanhua.com/catalogs/lianzai',
);
...
$log_id = C::t('#htt_baoman#htt_baoman_log')->insert($log_data,true);
...
foreach ($articles as $article) {
    ...
    $data = array();
    $data['content'] = pq($article)->find(".article-content h4 a")->text();
    $data['img'] = pq($article)->find(".article-content a img")->attr('data-original-image-url');

    var_dump($data);


    // $encode = mb_detect_encoding($data['content'], array('ASCII','GB2312','GBK','UTF-8');
    ...
    //记录ids。作为删除的标记。
    C::t('#htt_baoman#htt_baoman_log')->update($log_id,array('ids'=>$tid));
...
C::t('#htt_baoman#htt_baoman_log')->update($log_id,$log_data);
