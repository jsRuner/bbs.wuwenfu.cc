...
$plugin_lang = array(
    ...
    'password'=>'密码',
    'please_write_whole'=>'请输入完整信息',
);*/

$plugin_lang = $Plang = $scriptlang['htt_qqlogin'];
/*
$plugin_lang['show_export_title'] = "UID,OPENID,ACCESS_TOKEN,昵称,用户名,头像,注册时间";
$plugin_lang['show_export_btn'] = "导出QQ用户";
$plugin_lang['bind_user_by_admin'] = "请勿随意修改。如果修改则表示该QQ会登录对应的帐号。UID-用户名-密码要正确。用户可以手动绑定QQ注册。具体位置是 设置-用户资料-绑定QQ";


$plugin_lang['member_help'] = "<li>可以删除用户，则该用户点击QQ登录时会重新注册成一个会员。</li>
<li>通过编辑用户，可以实现老会员绑定QQ登录。操作步骤-老会员先执行QQ注册-编辑老会员记录,修改UID-username-password为之前的资料即可。</li>
<li>老用户可以自行绑定QQ登录。不建议管理员手动绑定。</li>
<li>插件卸载的时候,则请备份表或者导出记录。</li>
";*/
...
//        $add = '<input type="button" class="btn" onclick="location.href=\''.ADMINSCRIPT.'?action=plugins&operation=config&do='.$pluginid.'&identifier=htt_qqlogin&pmod=qq_member&ac=add\'" value="添加" />';
        $add = '';
        if($qqlogins) {
