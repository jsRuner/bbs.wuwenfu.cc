...
function get_img_content($img_url, $snoopy_obj = '', $args = array()){
	...
	if(!function_exists('fsockopen') && !function_exists('pfsockopen') || !$snoopy_obj){
		...
		$headers = (array)$snoopy_obj->headers;
		...
		if($header_arr['content-disposition']){
			...
			$info_arr = explode('filename=', $c_d);
			$arr[2] = preg_replace('(\'|\")', '', $info_arr[1]);//去掉引号
			...
			if(empty($file_name)){
				...
				$info['file_name'] =  str_iconv(urldecode(trim($patharr[count($patharr)-1])));
			}
			$info['content'] = $content;
			return $info;
		}else{
...
function dom_single_article($content = '',$dom_arr = ''){
	...
	$re['content'] = dom_get_str($html, $dom_arr['content'], array('is_get_all' => 0));//
