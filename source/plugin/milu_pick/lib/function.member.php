...
function get_web_avatar($args, $i = 0){
	...
	$img_content = file_get_contents($icon_url);
	$img_re['file_size'] = strlen($img_content);
	$show_arr = get_show_arr($now_get, $success_count, $get_count, $all_get_time);
...
function pick_get_user_info($args){
	...
	$dom_rules = 'h2.mt';
	if(check_discuz_version_for_member($content) == true) $dom_rules = 'div.hm h2.xs2 a';
	
	$user_info = dom_get_str($html, $dom_rules, array('is_get_all' => 1, 'text_type' => 'innertext', 'is_return_array' => 1));
	...
	if($username && $username_chinese == 1){
		...
		if($check_re == 'allen' && $username_chinese == 1) return -5;//用户名不是中文
	}	
...
function format_member_data($data_arr){
	foreach($data_arr as $k2 => $value){
		...
		if($k2 == 'birthyear#birthmonth#birthday'){//生日
			$re_arr['birthyear'] = pstrcut('a'.$value, 'a', milu_lang('year'));;
			$re_arr['birthmonth'] = pstrcut($value, milu_lang('year'), milu_lang('month'));
			$re_arr['birthday'] = pstrcut($value, milu_lang('month'), milu_lang('day'));
			unset($re_arr[$k2]);
		}
...
//返回true是x2类型的
function check_discuz_version_for_member($content){
	$find_str = '<h2 class="xs2">';
	if(strexists($content, $find_str)){
		return true;
	}else{
		return FALSE;
	}
