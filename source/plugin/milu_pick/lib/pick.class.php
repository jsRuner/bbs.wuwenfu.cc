...
class pick{
	...
	function get_redirect_attach($url, $content){
		...
		$format_args_content = array(
			...
			'show_type' => 'body',
			'test' => 2,
			'result_data' => $arr['attach_redirect_url'],
			'filter_html' => $content_filter_html,
			'replace_rules' => $this->p_arr['content_replace_rules'],
			'filter_data' => $this->p_arr['content_filter_rules'],
		);
		$arr['attach_redirect_url'] = filter_article($format_args_content);
		
		$format_args_content['result_data'] = $arr['attach_download_url'];
		
		$arr['attach_download_url'] = filter_article($format_args_content);
	...
	function get_article_result($content, $url = ''){
		...
		$info = $this->get_article($content, $url, 1);
	...
	function get_article($content, $url = '', $is_page = 0){
		global $_G;
		$url = $url ? $url : $this->now_url;
		$pick_config = $_G['cache']['evn_milu_pick']['pick_config'];
		...
		if($this->p_arr['rules_type'] == 3){//一键采集
			...
			if($is_page == 0 && !trim($article_info['title'])) return $article_info;//如果标题都取不到，不必浪费时间获取内容
		...
		if(VIP) {
			...
			if($this->pick_set['is_reply_hide_on'] == 1){
				if(!empty($article_info['content'])){
					$html = get_htmldom_obj($article_info['content']);
					if($html){
						foreach($html->find('div.showhide') as $v) {
							$showhide_in = $v->innertext;
							$showhide_out = $v->outertext;
							if($showhide_out){
								$article_info['content'] = str_replace($showhide_out, '[hide]'.$showhide_in.'[/hide]', $article_info['content']);
							}
						}
					}
				}
	...
	function format_article($article_info){
		...
		$article_info['content'] = $this->p_arr['is_html_public'] == 2 ? clear_ad_html($article_info['content']) : $article_info['content'];//去掉js、注释和框架等(不开启html的情况下才过滤)
