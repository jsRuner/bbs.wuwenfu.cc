(function(jQuery) {
	...
	register.checkNickName = function(obj){
		...
		if(nickName.length < 2){
			...
			jQuery("#input_nickname_s").html("昵称长度需两个字符以上");
		...
		jQuery.ajax({
			...
			success:function(response){
				...
				if(response=="register.fuhao.nickName"){
					jQuery("#input_nickname_s").html("用户名不合法");
					jQuery("#input_nickname").attr("class","reg-check-tip wrong");
					isNameOk = "F";
				}else if(response=="register.error.nickName"){
					jQuery("#input_nickname_s").html("包含有不允许注册的字符");
					jQuery("#input_nickname").attr("class","reg-check-tip wrong");
					isNameOk = "F";
				}else if(response=="register.check.nickName"){
					jQuery("#input_nickname_s").html("此昵称太受欢迎了，换一个试试。");
					jQuery("#input_nickname").attr("class","reg-check-tip wrong");
					isNameOk = "F";
				}else{
	...
	register.checkEmail = function(obj){
		...
		if(email.length <= 0){
			jQuery("#input_email_s").html("请输入登陆邮箱");
			jQuery("#input_email").show();
		...
		jQuery.ajax({
			...
			success:function(response){
				...
				if(response=="register.error.email"){
					jQuery("#input_email_s").html("这个怎么可能是正确邮箱呢？");
					jQuery("#input_email").attr("class","reg-check-tip wrong");
					isEmailOK = "F";
				}else if(response=="register.buyunxu.email"){
					jQuery("#input_email_s").html("天！这个邮箱不允许被注册");
					jQuery("#input_email").attr("class","reg-check-tip wrong");
					isEmailOK = "F";
				}else if(response=="register.beizhuche.email"){
					jQuery("#input_email_s").html("这个邮箱已经有人注册了哦");
					jQuery("#input_email").attr("class","reg-check-tip wrong");
					isEmailOK = "F";
				}else{
	...
	register.checkPw = function(obj){
		...
		if(password.length == 0){
			...
			jQuery("#input_password_s2").html("请输入密码").show();
			jQuery("#input_password").attr("class","reg-check-tip wrong");
			isPwdOK = "F";
		}else if(password.indexOf(" ") != -1){
			jQuery("#input_password_s").hide();
			jQuery("#input_password_s2").html("密码不能有违禁字符").show();
			jQuery("#input_password").attr("class","reg-check-tip wrong");
			isPwdOK = "F";
		}else if((password.length<6) || (password.length>24)){
			jQuery("#input_password_s").hide();
			jQuery("#input_password_s2").html("密码长度6到24位").show();
			jQuery("#input_password").attr("class","reg-check-tip wrong");
			isPwdOK = "F";
		}else{
			jQuery("#input_password_s").hide();
	...
	register.confirmPw = function(obj) {
		...
		if((password.length<6) || (password.length>24)) {
			...
			jQuery("#error_comfirm").html("两次输入的密码不一样,请重新输入!");
			jQuery("#input_passwordConfirm").show();
	...
	jQuery(document).on("focus","input[name='seccodeverify']",function() {
		jQuery("#verifycodeErrorMsg").html("请输入验证码");
			...
			jQuery.ajax({
				...
				success:function(response){
					...
					if(response=="register.error.validateCode"){
						jQuery("#verifycodeErrorMsg").html("验证码输入错误，请重新输入");
						jQuery("#input_verifycode").attr("class","reg-check-tip wrong");
						isVerifycodeOK = "F";
					}else{
	...
	jQuery(document).on("change","#birthprovince",function() {
		...
		var add = birthprovince.replace(/([省市])|(自治区)|(特别行政区)/,'');
		jQuery("#input_birthcity").show();
		if(birthprovince == ''){
			addbirth_wrong("请选择省份/地区","birthcity");
		} else {
			addbirth_next("请选择城市","birthcity");
		}
	});
	...
	jQuery(document).on("change","#birthcommunity",function() {
		...
		if(birthcommunity == ''){
			addbirth_wrong("请选择乡镇/街道","birthcity");
		} else {
			addbirth_ok("birthcity");
		}
	});
	...
	jQuery(document).on("change","#resideprovince",function() {
		...
		var add = resideprovince.replace(/([省市])|(自治区)|(特别行政区)/,'');
		jQuery("#input_residecity").show();
		if(resideprovince == ''){
			addbirth_wrong("请选择省份/地区","residecity");
		} else {
			addbirth_next("请选择城市","residecity");
		}
	});
	...
	jQuery(document).on("change","#residecommunity",function() {
		...
		if(residecommunity == ''){
			addbirth_wrong("请选择乡镇/街道","residecity");
		} else {
			addbirth_ok("residecity");
		}
	});
